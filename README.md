...
1. **Agent A 头脑风暴** - 生成 3 个差异化方向
2. **Agent B 逻辑审核** - 审查创意的逻辑自洽性
3. **等待用户选择** - 用户选择 1 个或多个方向
4. **循环迭代** - 基于用户选择，Agent A 生成详细创意，Agent B 审核
5. **用户确认** - 用户满意后，Agent C 进行爆款评估
6. **生成大纲** - 评估通过后生成最终故事大纲

#### 内置判断函数

```python
# 是否处于方向选择阶段
engine.is_direction_selection_phase() -> bool
...
**首次输出**：仅生成 3 个差异化方向（不含详细创意）

**后续输出**：基于用户选择，生成详细创意，包括：
- 主角雏形（身份/背景、核心特质、内在矛盾）
...
3. 点击「新建对话」，输入故事主题
4. 等待 Agent A 生成 3 个方向
5. 输入「方向1」或「方向1、2」选择方向
6. Agent A 生成详细创意，Agent B 审核
7. 满意后输入「满意」或「可以」
8. Agent C 评估后，生成最终大纲
